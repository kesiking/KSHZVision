...
static CGFloat const SVPullToRefreshViewHeight = 78;
...
- (void)addPullToRefreshWithActionHandler:(void (^)(void))actionHandler {
    ...
    if(!self.pullToRefreshView) {
        ...
        view.textColor=[UIColor colorWithWhite:102/255.0f alpha:1.0f];
...
// public properties
@synthesize pullToRefreshActionHandler, textColor, activityIndicatorViewStyle, lastUpdatedDate, dateFormatter;
...
//    self.arrow.frame = arrowFrame;
    ...
    if(subtitleSize.height > 0){
        ...
        self.titleLabel.frame = CGRectIntegral(CGRectMake(labelX, titleY, maxLabelWidth, titleSize.height));
        self.subtitleLabel.frame = CGRectIntegral(CGRectMake(labelX, titleY + titleSize.height + marginY, maxLabelWidth, subtitleSize.height));
    }else{
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {    
    BOOL isClipsToBounds = contentOffset.y > 0 ? NO : YES;
    if (isClipsToBounds != self.scrollView.clipsToBounds) {
        self.scrollView.clipsToBounds = isClipsToBounds;
    }
    BOOL isHiddenArrow = contentOffset.y > 0 ? YES : NO;
    if (isHiddenArrow != self.arrow.hidden) {
        self.arrow.hidden      = isHiddenArrow;
        self.titleLabel.hidden = isHiddenArrow;
    }
...
- (UIImageView *)arrow {
    if(!_arrow) {
        _arrow = [[UIImageView alloc]initWithFrame:CGRectMake(0, self.bounds.size.height-54-18, 32, 32)];
        CGPoint point= _arrow.center;
        point.x=self.bounds.size.width/2;
        _arrow.center=point;
        _arrow.contentMode = UIViewContentModeCenter;
        _arrow.backgroundColor = [UIColor clearColor];
        _arrow.image = [UIImage imageNamed:@"tb_icon_all_loading_pulldown"],
...
- (UILabel *)titleLabel {
    if(!_titleLabel) {
        ...
        _titleLabel.text = NSLocalizedString(@"Pull down to refresh...",);
...
- (UILabel *)subtitleLabel {
    if(!_subtitleLabel) {
        ...
        CGPoint point= _subtitleLabel.center;
        point.x=self.bounds.size.width/2;
        _subtitleLabel.center=point;
...
- (UIColor *)arrowColor {
	return self.arrow.backgroundColor; // pass through
}
...
- (void)setArrowColor:(UIColor *)newArrowColor {
	self.arrow.backgroundColor = newArrowColor; // pass through
