//
//  WeAppBasicSelectorScrollView.m
//  WeAppSDK
//
...
@interface WeAppBasicSelectorScrollView ()
...
@implementation WeAppBasicSelectorScrollView
...
- (void)setPluginCnt:(NSUInteger)pluginCnt defaultIndex:(NSUInteger)defaultIndex{
    ...
    for (UIView * subview in self.subviews) {
        if ([subview isKindOfClass:[WeAppBasicMenuItemView class]]) {
    ...
    for (; i < end; i++) {
        WeAppBasicMenuItemView * btn = nil;
        if (self.itemViewClass) {
            btn = [[self.itemViewClass alloc] init];
        }else{
            btn = [[WeAppBasicMenuItemView alloc] init];
        }
...
- (void)reloadData{
    for (WeAppBasicMenuItemView *btn in self.btnArray) {
...
- (void)btnClicked:(id)sender {
    @try {
        ...
        if (![view isKindOfClass:[WeAppBasicMenuItemView class]]) {
            return;
        }
        ...
        WeAppBasicMenuItemView * btn = (WeAppBasicMenuItemView * )view;
        [_scrollView setContentOffset:CGPointMake(_scrollView.contentOffset.x, 0) animated:YES];
        
        /*******************
         屏蔽点击过快的情况，避免bug
         *******************/
        WeAppBasicMenuItemView *oldBtn = (WeAppBasicMenuItemView*)[self.btnArray objectAtIndex:_defaultIndex];
...
- (void)btnClickedDown:(id)sender{
    ...
    [self itemViewClickedDown:(WeAppBasicMenuItemView *)view];
}
...
- (void)btnClickedTouchCancel:(id)sender{
    ...
    [self itemViewClickedCancel:(WeAppBasicMenuItemView *)view];
}
...
- (void)setPluginSelectBtn:(NSUInteger)index{
    ...
    WeAppBasicMenuItemView * btn = (WeAppBasicMenuItemView*)[self.btnArray objectAtIndex:index];
    WeAppBasicMenuItemView *oldBtn = (WeAppBasicMenuItemView*)[self.btnArray objectAtIndex:_defaultIndex];
...
- (WeAppBasicMenuItemView*)getSelectorWithIndex:(NSUInteger)index{
...
- (NSInteger)getIndexWithSelector:(WeAppBasicMenuItemView*)itemView{
    if (!self.btnArray || [self.btnArray count] <= _defaultIndex) {
        return -1;
    }
    if (itemView && [itemView isKindOfClass:[WeAppBasicMenuItemView class]] && [self.btnArray containsObject:itemView]) {
...
- (void)reloadSelecorPosition{
    ...
    WeAppBasicMenuItemView *nowBtn = (WeAppBasicMenuItemView*)[self.btnArray objectAtIndex:_defaultIndex];
    
    if (_defaultIndex < [self.btnArray count] - 1 && _defaultIndex > 0) {
        WeAppBasicMenuItemView *nextItemView = (WeAppBasicMenuItemView*)[self.btnArray objectAtIndex:(_defaultIndex + 1)];
...
-(void)setSelectorProperty:(WeAppBasicMenuItemView*)itemView withIndex:(NSUInteger)index isSelect:(BOOL)isSelect{
    //没有传入过array的话什么也不操作
    return;
}
...
-(void)setSelectorFrame:(NSUInteger)pluginCnt defaultIndex:(NSUInteger)defaultIndex{
    ...
    self.buttonWidth = [WeAppButtonWidthAlgorithm viewDynamicWidthWithNum:(pluginCnt)];
    self.buttonHeight = [WeAppButtonWidthAlgorithm viewDynamicHeightWithNum:(pluginCnt)];
}

//Selector被选中时触发的事件，返回新选中的itemView与之前的itemView，如果需要做变化就继承该函数
-(BOOL)itemViewSelected:(WeAppBasicMenuItemView*)itemView withOldItemView:(WeAppBasicMenuItemView*)oldItemView{
    return YES;
}

//Selector被选中时触发的事件，返回新选中的itemView以及index值
-(void)itemViewSelected:(WeAppBasicMenuItemView*)itemView atIndex:(NSUInteger)index{
    
}


//Selector被按下时的操作,用于改变按下时的样式
-(void)itemViewClickedDown:(WeAppBasicMenuItemView*)itemView{
    
}

//Selector被放弃选中时的操作,用于改变放弃时的样式
-(void)itemViewClickedCancel:(WeAppBasicMenuItemView*)itemView{
...
-(void)scrollViewActionWithNewItemView:(WeAppBasicMenuItemView*)itemView withOldItemView:(WeAppBasicMenuItemView*)oldItemView{
    ...
    if (newBtnInteger >= oldBtnInteger) {
        ...
        if (newBtnInteger < [self.btnArray count] - 1 && isSubviewInScrollView) {
            WeAppBasicMenuItemView *nextItemView = (WeAppBasicMenuItemView*)[self.btnArray objectAtIndex:(newBtnInteger + 1)];
